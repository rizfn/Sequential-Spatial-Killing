...
EXECUTABLE="$SCRIPT_DIR/finalMass"
...
for N_colors in "${N_COLORS_VALUES[@]}"; do
    for K_selections in "${K_SELECTIONS_VALUES[@]}"; do
        if [ $K_selections -le $INITIAL_TOTAL_BALLS ]; then
            echo "$N_colors $K_selections $N_STEPS $INITIAL_TOTAL_BALLS" >> "$PARAM_FILE"
        fi
    done
done
...
if command -v parallel &> /dev/null; then
    ...
    cat "$PARAM_FILE" | parallel --progress -j $MAX_JOBS --colsep ' ' "$EXECUTABLE {1} {2} {3} {4}"
else
