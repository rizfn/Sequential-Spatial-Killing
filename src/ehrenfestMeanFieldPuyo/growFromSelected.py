...
def simulate_urn(N_colors, K_selections, N_steps, initial_total_balls):
    ...
    for step in range(N_steps):
        ...
        if len(unique_colors) == K_selections:
            ...
            random_color = np.random.choice(unique_colors)
            urn[random_color] += 1
        else:
...
def main():
    # Parameter ranges
    N_colors_range = np.arange(2, 1000, 8)
    K_selections_range = np.arange(2, 20, 1)
    N_steps = 10000
    initial_total_balls = 2000
    ...
    plt.savefig(f"src/ehrenfestMeanFieldPuyo/plots/massVsTime/growFromSelected_steps_{N_steps}_init_{initial_total_balls}.png", dpi=300)
    print(f"Heatmap saved to src/ehrenfestMeanFieldPuyo/plots/massVsTime/growFromSelected_steps_{N_steps}_init_{initial_total_balls}.png")
