...
def main():
    ...
    # plt.plot(N_colors_range, 1.18*np.sqrt(np.array(N_colors_range)), color='black', linestyle='--')
    plt.tight_layout()
    
    os.makedirs("src/ehrenfestMeanFieldPuyo/plots/massVsTime", exist_ok=True)
    plt.savefig(f"src/ehrenfestMeanFieldPuyo/plots/massVsTime/heatmap_steps_{N_steps}_init_{initial_total_balls}.png", dpi=300)
    print(f"Heatmap saved to src/ehrenfestMeanFieldPuyo/plots/massVsTime/heatmap_steps_{N_steps}_init_{initial_total_balls}.png")
