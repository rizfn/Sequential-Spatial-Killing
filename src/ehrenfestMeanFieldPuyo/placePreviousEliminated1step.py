...
def simulate_urn(N_colors, K_selections, N_steps, initial_total_balls):
    ...
    for step in range(N_steps):
        ...
        if len(unique_colors) == K_selections:
            ...
            # Add the color that was eliminated in the previous step
            if last_eliminated_color is not None:
                random_color = last_eliminated_color
                last_eliminated_color = None  # Reset after using it once
            else:
                # No recent elimination, choose randomly
...
def main():
    ...
    plt.savefig(f"src/ehrenfestMeanFieldPuyo/plots/massVsTime/prevEliminatedStrategyOnestep_steps_{N_steps}_init_{initial_total_balls}.png", dpi=300)
    print(f"Heatmap saved to src/ehrenfestMeanFieldPuyo/plots/massVsTime/prevEliminatedStrategyOnestep_steps_{N_steps}_init_{initial_total_balls}.png")
