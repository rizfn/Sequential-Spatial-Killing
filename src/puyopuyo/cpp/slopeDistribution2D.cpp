...
void run(std::ofstream &file, int L, int N_SPECIES, int STEPS_PER_LATTICEPOINT)
{
    ...
    for (int step = 0; step <= STEPS_PER_LATTICEPOINT; ++step)
    {
        ...
        // Write the step, absolute height of the first column, and local slopes
        file << step << "\t" << heights[0] << "\t"; // Write the step and the absolute height of the first column
        for (int col = 0; col < L - 1; ++col)
        {
            int slope = heights[col + 1] - heights[col];
            file << slope;
            if (col < L - 2)
                file << ",";
        }
...
int main(int argc, char *argv[])
{
    ...
    filePathStream << exeDir << "\\outputs\\slopeDistribution2D\\L_" << L << "_N_" << N_SPECIES << "_steps_" << STEPS_PER_LATTICEPOINT << ".tsv";
    std::string filePath = filePathStream.str();

    std::ofstream file;
    file.open(filePath);
    file << "step\tfirstColumnHeight\tlocalSlopes\n";
