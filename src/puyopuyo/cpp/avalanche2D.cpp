...
int annihilatePuyo(std::vector<std::vector<int>> &lattice, std::vector<std::vector<bool>> &movedSites, int L, int H, int &avalancheCount)
{
...
void run(std::ofstream &file, int L, int N_SPECIES, int STEPS_PER_LATTICEPOINT)
{
    ...
    for (int i = 0; i < L * STEPS_PER_LATTICEPOINT; ++i)
    {
        ...
        file << std::fixed << std::setprecision(6) << step << "\t" << filled_cells << "\t" << max_height << "\t"
             << avalancheCount << "\t" << totalEliminated << "\n";
...
int main(int argc, char *argv[])
{
    ...
    std::ofstream file;
    file.open(filePath);
    file << "step\tmass\theight\tavalanches\ttotal_eliminated\n";
