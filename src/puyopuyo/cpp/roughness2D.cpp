...
double calculateSurfaceRoughness(const std::vector<std::vector<int>> &lattice, int L, int H)
{
...
void run(std::ofstream &file, int L, int N_SPECIES, int STEPS_PER_LATTICEPOINT)
{
    ...
    for (int step = 0; step <= STEPS_PER_LATTICEPOINT; ++step)
    {
        ...
        // Record both the number of filled cells, the max height of any column, and the surface roughness
        int filled_cells = 0;
        int max_height = 0;
        for (int col = 0; col < L; ++col)
        {
        ...
        double roughness = calculateSurfaceRoughness(lattice, L, H);

        // Record the number of filled cells, the max height, and the surface roughness
        file << step << "\t" << filled_cells << "\t" << max_height << "\t" << roughness << "\n";
...
int main(int argc, char *argv[])
{
    ...
    std::ofstream file;
    file.open(filePath);
    file << "step\tmass\theight\troughness\n";
