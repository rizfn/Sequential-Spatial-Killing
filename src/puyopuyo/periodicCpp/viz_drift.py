...
def drift_vs_entropy():
    ...
    for ax, dim in zip(axes, dims):
        ...
        for N in N_list:
            ...
            if dim == 1:
                ...
                L = Ls[dim]
                if N == int(N):
                    fn = f"src/puyopuyo/periodicCpp/outputs/gravity{dim}D/L_{L}_N_{int(N)}_steps_{steps}.tsv"
                else:
                    fn = f"src/puyopuyo/periodicCpp/outputs/gravity{dim}D/L_{L}_N_{N}_steps_{steps}.tsv"
                data = np.loadtxt(fn, delimiter="\t", skiprows=1, unpack=True)
                step, mass = data[0], data[1]
    ...
    plt.tight_layout()
    plt.savefig(f"src/puyopuyo/periodicCpp/plots/drift/driftVsInvEntropy.png", dpi=300)
    plt.show()
