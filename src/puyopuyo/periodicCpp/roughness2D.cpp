...
double calculateSurfaceRoughness(const std::vector<std::vector<int>> &lattice, int L, int H, std::vector<int> &heights)
{
    heights.assign(L, 0);
    ...
    // Calculate the arithmetic average roughness (Ra)
    ...
    for (int height : heights)
        roughness += std::abs(height - meanHeight);
    roughness /= L;

    return roughness;
}

double calculateAverageLocalSlope(const std::vector<int> &heights, int L)
{
    double totalSlope = 0.0;
    for (int col = 0; col < L; ++col)
    {
        int nextCol = (col + 1) % L; // periodic boundary
        totalSlope += std::abs(heights[col] - heights[nextCol]);
    }
    return totalSlope / L;
}
...
void run(std::ofstream &file, int L, int N_SPECIES, int STEPS_PER_LATTICEPOINT)
{
    ...
    for (int step = 0; step <= STEPS_PER_LATTICEPOINT; ++step)
    {
        ...
        double roughness = calculateSurfaceRoughness(lattice, L, H, heights);
        double avgLocalSlope = calculateAverageLocalSlope(heights, L);

        file << step << "\t" << roughness << "\t" << avgLocalSlope << "\n";
...
int main(int argc, char *argv[])
{
    ...
    filePathStream << exeDir << "\\outputs\\roughness2D\\L_" << L << "_N_" << N_SPECIES << "_steps_" << STEPS_PER_LATTICEPOINT << ".tsv";
    std::string filePath = filePathStream.str();

    std::ofstream file;
    file.open(filePath);
    file << "step\troughness\tavg_local_slope\n";
