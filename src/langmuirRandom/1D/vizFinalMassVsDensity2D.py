...
def main():
    ...
    files = glob(f"src/langmuirRandom/1D/outputs/finalMass/L_{L}_N_{N}/rho_*_steps_{STEPS}_sim_*.txt")
    ...
    plt.scatter(densities, np.array(final_masses) / (STEPS * L), alpha=0.3, s=30)
    plt.xlabel(r'Density ($\rho$)', fontsize=12)
    plt.ylabel('Mass added per step per site', fontsize=12)
    ylim = plt.ylim()
    plt.plot(densities, 1 - 2*np.array(densities), '--', lw=2, label='1 - 2*rho', c='#902040')
    plt.ylim(ylim)
    plt.title(f'Drift Velocity vs Density (L={L}, N={N}, STEPS={STEPS})', fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    output_path = f'src/langmuirRandom/1D/plots/finalMass_vs_density/L{L}_N{N}_steps{STEPS}.png'
